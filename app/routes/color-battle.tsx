...
export function meta() {
  return [
    { title: "Color Battle - Arena" },
    { name: "description", content: "Input juara Color Battle" },
  ];
}

export default function ColorBattle() {
  const { data, setRank, updateTeamName, resetData } = useRankedGame('color_battle_data');
  const scores = data.matches[0]?.scores || {};

  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="text-center mb-12">
        <h1 className="text-5xl font-black text-yellow-600 italic uppercase tracking-tighter drop-shadow-sm">
          COLOR <span className="text-gray-900 underline decoration-yellow-400">BATTLE</span>
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        {/* Input Juara */}
        <div className="bg-white p-8 rounded-[2rem] shadow-xl border-4 border-yellow-100">
          <h2 className="text-2xl font-black text-gray-900 mb-8 uppercase flex items-center gap-3">
            <span className="bg-yellow-400 text-white w-10 h-10 rounded-xl flex items-center justify-center">🥇</span>
            Input Pemenang
          </h2>
            ...
            {[1, 2, 3].map((rank) => (
              <div key={rank} className="group">
                ...
                <select
                  ...
                  className="w-full bg-yellow-50 border-2 border-yellow-200 rounded-2xl px-6 py-4 text-lg font-bold text-gray-900 focus:ring-4 focus:ring-yellow-400 focus:border-yellow-400 outline-none transition-all appearance-none cursor-pointer"
                >
        ...
        <div className="space-y-8">
          <div className="bg-white p-8 rounded-[2rem] shadow-xl border-4 border-gray-100 min-h-[400px]">
            <ScoreChart teams={data.teams} color="#eab308" titleColor="#854d0e" maxY={10} />
          </div>
          
          <div className="bg-gray-900 p-8 rounded-[2rem] shadow-2xl text-white">
            <h3 className="text-xl font-black mb-6 uppercase tracking-widest text-yellow-400">Hasil Poin</h3>
            <div className="grid grid-cols-2 gap-4">
              {data.teams.map(team => (
                <div key={team.id} className="bg-white/5 p-4 rounded-xl flex justify-between items-center border border-white/10">
                  ...
                  <span className="font-black text-yellow-400">{team.score}</span>
                </div>
              ))}
            </div>
          </div>
